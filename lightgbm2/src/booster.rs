...
pub struct Booster {
    handle: lightgbm2_sys::BoosterHandle,
}

impl Booster {
    fn new(handle: lightgbm2_sys::BoosterHandle) -> Self {
        Booster { handle }
    }
    ...
    pub fn from_file(filename: &str) -> Result<Self> {
        ...
        lgbm_call!(lightgbm2_sys::LGBM_BoosterCreateFromModelfile(
    ...
    pub fn train(dataset: Dataset, parameter: &Value) -> Result<Self> {
        ...
        let mut handle = std::ptr::null_mut();
        lgbm_call!(lightgbm2_sys::LGBM_BoosterCreate(
            dataset.handle,
        ...
        for _ in 1..num_iterations {
            lgbm_call!(lightgbm2_sys::LGBM_BoosterUpdateOneIter(
                handle,
                &mut is_finished
            ))?;
        }
    ...
    pub fn predict(&self, data: Vec<Vec<f64>>) -> Result<Vec<Vec<f64>>> {
        ...
        // get num_class
        let mut num_class = 0;
        lgbm_call!(lightgbm2_sys::LGBM_BoosterGetNumClasses(
            self.handle,
            &mut num_class
        ))?;
        ...
        lgbm_call!(lightgbm2_sys::LGBM_BoosterPredictForMat(
            self.handle,
            flat_data.as_ptr() as *const c_void,
            lightgbm2_sys::C_API_DTYPE_FLOAT64 as i32,
            data_length as i32,
    ...
    /// Get Feature Num.
    pub fn num_feature(&self) -> Result<usize> {
        let mut out_len = 0;
        lgbm_call!(lightgbm2_sys::LGBM_BoosterGetNumFeature(
            self.handle,
            &mut out_len
        ))?;
    ...
    pub fn feature_name(&self) -> Result<Vec<String>> {
        ...
        let out_strs = (0..num_feature)
            ...
            .collect::<Vec<_>>();
        lgbm_call!(lightgbm2_sys::LGBM_BoosterGetFeatureNames(
            self.handle,
    ...
    pub fn feature_importance(&self) -> Result<Vec<f64>> {
        ...
        lgbm_call!(lightgbm2_sys::LGBM_BoosterFeatureImportance(
            self.handle,
            0_i32,
            0_i32,
            out_result.as_ptr() as *mut c_double
    ...
    pub fn save_file(&self, filename: &str) -> Result<()> {
        let filename_str = CString::new(filename).unwrap();
        lgbm_call!(lightgbm2_sys::LGBM_BoosterSaveModel(
            self.handle,
            0_i32,
            -1_i32,
            0_i32,
...
impl Drop for Booster {
    fn drop(&mut self) {
        lgbm_call!(lightgbm2_sys::LGBM_BoosterFree(self.handle)).unwrap();
    }
}
