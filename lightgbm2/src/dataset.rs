...
/// ```
pub struct Dataset {
    pub(crate) handle: lightgbm2_sys::DatasetHandle,
}

#[link(name = "c")]
impl Dataset {
    fn new(handle: lightgbm2_sys::DatasetHandle) -> Self {
        Self { handle }
    }
    ...
    pub fn from_mat(data: Vec<Vec<f64>>, label: Vec<f32>) -> Result<Self> {
        ...
        lgbm_call!(lightgbm2_sys::LGBM_DatasetCreateFromMat(
        ...
        lgbm_call!(lightgbm2_sys::LGBM_DatasetSetField(
            handle,
            label_str.as_ptr() as *const c_char,
            label.as_ptr() as *const c_void,
            data_length as i32,
            lightgbm2_sys::C_API_DTYPE_FLOAT32 as i32
        ))?;
    ...
    pub fn from_file(file_path: &str) -> Result<Self> {
        ...
        lgbm_call!(lightgbm2_sys::LGBM_DatasetCreateFromFile(
...
impl Drop for Dataset {
    fn drop(&mut self) {
        lgbm_call!(lightgbm2_sys::LGBM_DatasetFree(self.handle)).unwrap();
    }
}
